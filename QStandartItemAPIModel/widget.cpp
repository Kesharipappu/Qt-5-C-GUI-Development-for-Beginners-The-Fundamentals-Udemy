    ...
    , ui(new Ui::Widget)
{
    ...
    QStandardItem *rawandaItem = new QStandardItem("Rawanda");
    ...
    africaItem->appendRow(rawandaItem);
    africaItem->appendRow(ugandaItem);

    rawandaItem->appendRow(kigaliItem);
    rawandaItem->appendRow(musanzeItem);
...
void Widget::selectionsChangesSlot(const QItemSelection &oldSelection, const QItemSelection &newSelection)
{
